...
class des():
    ...
    def run(self, key, text, action=ENCRYPT, padding=False):
        ...
        print(len(text_blocks), text_blocks)
        result = list()
        for block in text_blocks:#Loop over all the blocks of data
            ...
            for i in range(16): #Do the 16 rounds
                ...
                print("\nvalue after Expansion Permutation")
                print(d_e)
                if action == ENCRYPT:
                    tmp = self.xor(self.keys[i], d_e)#If encrypt use Ki
                else:
                    tmp = self.xor(self.keys[15-i], d_e)#If decrypt start by the last key
                print("value after xor with key\n")
                print(tmp)
                tmp = self.substitute(tmp) #Method that will apply the SBOXes
                print("value after keyed substitute\n")
                print(tmp)
                tmp = self.permut(tmp, P)
                print("value after Transposition\n")
                print(tmp)
                tmp = self.xor(g, tmp)
                print("value after xor with left section\n")
                print(tmp)
                g = d
                d = tmp
    ...
    def generatekeys(self):#Algorithm that generates all the keys
        ...
        print("After Initial permut---=====----   ", key)
        g, d = nsplit(key, 28) #Split it in to (g->LEFT),(d->RIGHT)
        print("Divided into 2 parts\n")
        print(g,d)
        
        for i in range(16):#Apply the 16 rounds
            g, d = self.shift(g, d, SHIFT[i]) #Apply the shift associated with the round (not always 1)
            print("Round ", i)
            print("After Left cercular Shift")
            print("Left part : ", g)
            print("Right part : ", d)
            
            tmp = g + d #Merge them
            print("After Permut:", self.permut(tmp, CP_2))
